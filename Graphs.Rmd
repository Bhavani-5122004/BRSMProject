...
aggregated_matrix <- raw_dataset %>%
  ...
  summarise(
    ...
    # IR components
    Total_Repeats = sum(isRepeat == TRUE, na.rm = TRUE),
...
ggplot(aggregated_matrix, aes(x = IR_Memorability)) +
  ...
  facet_grid(Sentence_Voice ~ Word_Condition) +
  labs(
    title = "IR Memorability Distribution by Word Condition and Voice",
    x = "IR Memorability",
    y = "Count"
  ) +
  theme_minimal()
...
ggplot(aggregated_matrix, aes(x = WR_Accuracy)) +
  ...
  facet_grid(Sentence_Voice ~ Word_Condition) +
  labs(
    title = "Distribution of WR Accuracy Scores by Condition",
    x = "WR Accuracy",
    y = "Count"
  ) +
  theme_minimal()
...
fisch_data <- aggregated_matrix %>%
  mutate(
    IR_Correct = ifelse(IR_Memorability > 0, "Correct", "Incorrect"),
    WR_Correct = ifelse(WR_Accuracy > 0.5, "Correct", "Incorrect")
  )

ir_word_table <- table(fisch_data$Word_Condition, fisch_data$IR_Correct)
chisq_ir_word <- fisher.test(ir_word_table)
print(chisq_ir_word)

wr_word_table <- table(fisch_data$Word_Condition, fisch_data$WR_Correct)
chisq_wr_word <- fisher.test(wr_word_table)
print(chisq_wr_word)

ir_voice_table <- table(fisch_data$Sentence_Voice, fisch_data$IR_Correct)
chisq_ir_voice <- fisher.test(ir_voice_table)
print(chisq_ir_voice)

wr_voice_table <- table(fisch_data$Sentence_Voice, fisch_data$WR_Correct)
chisq_wr_voice <- fisher.test(wr_voice_table)
print(chisq_wr_voice)
```

```{r}
# Create categorical correctness variables
chi_data <- aggregated_matrix %>%
  mutate(
    IR_Correct = ifelse(IR_Memorability > 0, "Correct", "Incorrect"),
    WR_Correct = ifelse(WR_Accuracy > 0.5, "Correct", "Incorrect")
  )

# Perform Chi-square tests to observe whether correctness (IR and WR) is associated with word condition and sentence voice
ir_word_table <- table(chi_data$Word_Condition, chi_data$IR_Correct)
chisq_ir_word <- chisq.test(ir_word_table)
print(chisq_ir_word)

wr_word_table <- table(chi_data$Word_Condition, chi_data$WR_Correct)
chisq_wr_word <- chisq.test(wr_word_table)
print(chisq_wr_word)

ir_voice_table <- table(chi_data$Sentence_Voice, chi_data$IR_Correct)
chisq_ir_voice <- chisq.test(ir_voice_table)
print(chisq_ir_voice)

wr_voice_table <- table(chi_data$Sentence_Voice, chi_data$WR_Correct)
...
ir_wr_counts <- raw_dataset %>%
  summarise(
    ...
    IR_Correct = sum(Event == "IR pressed" & Accuracy.IR == 1, na.rm = TRUE),
    IR_Incorrect = sum(Event == "IR pressed" & Accuracy.IR == 0, na.rm = TRUE),

    WR_Total = sum(Event == "WR pressed", na.rm = TRUE),
    WR_Correct = sum(Event == "WR pressed" & Accuracy.WR == 1, na.rm = TRUE),
    WR_Incorrect = sum(Event == "WR pressed" & Accuracy.WR == 0, na.rm = TRUE)
  )
...
Tejasvini: Organizing the graphs PLotting Graphs