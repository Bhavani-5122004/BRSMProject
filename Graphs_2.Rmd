...
long_data <- aggregated_scaled %>%
  drop_na(Word_Condition) %>%
  pivot_longer(
    cols = c(IR_scaled, WR_scaled),
    names_to = "Metric",
    values_to = "Score"
  ) %>%
  mutate(
    Metric = ifelse(Metric == "IR_scaled", "IR", "WR"),
    Word_Condition = as.factor(Word_Condition),
    Metric = as.factor(Metric),
    Score = as.numeric(Score)
  )

perm_test <- independence_test(
  Score ~ Word_Condition * Metric,
  data = long_data,
  distribution = approximate(nresample = 10000)
)

print("Permutation test to check if there is a difference in impact of word condition on the metrics:")
perm_test

# checking if there's a bigger impact of word condition on IR than WR
effect_size <- function(data, response) {
  total_mean <- mean(data[[response]], na.rm = TRUE)
  ss_total <- sum((data[[response]] - total_mean)^2, na.rm = TRUE)
  ss_between <- data %>%
    group_by(Word_Condition) %>%
    summarise(mean_val = mean(.data[[response]], na.rm = TRUE),
              n = n(), .groups="drop") %>%
    summarise(sum(n * (mean_val - total_mean)^2)) %>% pull()
  return(ss_between / ss_total)
}

ir_eta <- effect_size(aggregated_scaled, "IR_scaled")
wr_eta <- effect_size(aggregated_scaled, "WR_scaled")

ir_eta
wr_eta
print("Difference in effect sizes - positive indicates higher effect on IR")
ir_eta - wr_eta
...
for (i in 1:n_perm) {
  perm_data <- aggregated_scaled %>%
    mutate(Word_Condition = sample(Word_Condition))
  ir_perm <- effect_size(perm_data, "IR_scaled")
  wr_perm <- effect_size(perm_data, "WR_scaled")
  diffs[i] <- ir_perm - wr_perm
}

obs_diff <- ir_eta - wr_eta
p_value <- mean(abs(diffs) >= abs(obs_diff))
print("p-value to check if the increase is significant:")
p_value
...
long_voice <- aggregated_scaled %>%
  ...
  mutate(
    Metric = ifelse(Metric == "IR_scaled", "IR", "WR"),
    Sentence_Voice = as.factor(Sentence_Voice),
    Metric = as.factor(Metric),
    Score = as.numeric(Score)
  )

voice_perm_test <- independence_test(
  Score ~ Sentence_Voice * Metric,
  data = long_voice,
  distribution = approximate(nresample = 10000)
)

print("Permutation test to check if there is a difference in impact of sentence voice on the metrics:")
voice_perm_test
